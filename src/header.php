...
<div id="page" class="site">
	...
	<header class="sticky top-0 z-50 w-full border-b border-solid border-[#e7f3ea] dark:border-[#1d3a24] bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-md">
		<div class="max-w-[1280px] mx-auto flex items-center justify-between px-6 py-2 lg:py-4">
			<div class="flex gap-3 items-center">
				<?php if ( has_custom_logo() ) : ?>
					<div class="w-32 h-16 text-primary dark:text-accent lg:w-40 lg:h-20">
						<?php the_custom_logo(); ?>
					</div>
				<?php else : ?>
					<div class="text-accent dark:text-primaryt size-6 lg:size-8">
			...
			<nav class="hidden gap-6 items-center px-4 lg:flex">
				<?php
				wp_nav_menu(
					array(
			...
			<div class="flex gap-4 items-center">
				...
				<label class="lg:hidden cursor-pointer p-1 z-[70]" for="mobile-menu-toggle">
					<span class="text-2xl lg:text-3xl material-symbols-outlined mobile-menu-icon text-primary dark:text-white">menu</span>
					<span class="hidden text-2xl text-white lg:text-3xl material-symbols-outlined mobile-close-icon">close</span>
				</label>
			</div>
		</div>
	</header>
	...
	<div class="lg:hidden fixed top-0 bottom-0 right-0 bg-white dark:bg-background-dark z-[99999] transform translate-x-full transition-transform duration-300 ease-in-out overflow-hidden w-[70%] max-w-sm shadow-2xl flex flex-col" id="mobile-nav">
		...
		<div class="flex-shrink-0 bg-white dark:bg-background-dark border-b border-[#e7f3ea] dark:border-[#1d3a24] px-6 py-2 flex items-center justify-between">
			<div class="flex gap-3 items-center">
				<?php if ( has_custom_logo() ) : ?>
					<div class="w-32 h-16 text-primary dark:text-accent">
						<?php the_custom_logo(); ?>
					</div>
				<?php else : ?>
			...
			<button type="button" class="p-2 rounded-full transition-colors close-mobile-menu hover:bg-gray-100 dark:hover:bg-gray-800" aria-label="Close menu">
		...
		<div class="flex overflow-hidden flex-col flex-1">
			...
			<nav class="overflow-y-auto flex-1 w-full min-h-0">
				<?php
				wp_nav_menu(
					array(
						'theme_location' => 'primary',
			...
			<div class="flex-shrink-0 mt-6 pt-4 px-6 border-t border-[#e7f3ea] dark:border-[#1d3a24]">
				<a href="<?php echo esc_url( home_url( '/partners' ) ); ?>" class="w-full bg-primary dark:bg-accent text-white hover:text-white py-3 rounded-lg font-bold text-sm shadow-lg shadow-primary/20 hover:shadow-primary/40 hover:scale-[1.02] transition-all flex items-center justify-center">
					Partner with Us
				</a>
				<p class="my-4 text-xs font-semibold tracking-widest text-center text-gray-500 uppercase dark:text-gray-400">
